...
- The ZED 2i enumerates as a plain UVC device at `/dev/video0` on the Orin
  ...
  useful driving mode is `2560x720@30`, cropped to a single `1280x720@30` eye.
  Capture therefore needs neither the ZED SDK nor `zed_ros2_wrapper`, which
  keeps this path independent of the localization stack that owns them.
...
`videocrop right=1280` keeps the left eye. `key-int-max=30` bounds recovery
after loss to about one second. `config-interval=1` repeats SPS/PPS so a
receiver that joins late, or after a loss burst, can start decoding without
the stream being restarted.

Node parameters bound what a request may ask for: maximum resolution,
maximum bitrate, and the allowed capture device. An out-of-range or malformed
request is rejected into the `failed` state and never applied — the rover
does not let the ground station ask for anything the link cannot carry.
