...
def test_save_then_list_then_load_round_trips(tmp_path):
    ...
    loaded, voxels, voxel_m = store.load('yard-day1')
...
def test_save_then_load_round_trips_the_obstacle_voxels(tmp_path):
    ...
    _, voxels, _ = store.load('yard-day1')
    assert voxels.dtype == np.int32
    assert np.array_equal(voxels, saved_voxels)


def test_save_then_load_round_trips_the_obstacle_voxel_size(tmp_path):
    store = MapStore(str(tmp_path))
    store.save('yard-day1', state(), np.array([[1, 2, 3]], dtype=np.int32), voxel_m=0.10)

    _, _, voxel_m = store.load('yard-day1')
    assert voxel_m == 0.10
...
def test_loading_a_map_saved_before_voxels_existed_falls_back_to_empty(tmp_path):
    ...
    _, voxels, voxel_m = store.load('old')
    assert voxels.shape == (0, 3) and voxels.dtype == np.int32
    assert voxel_m == 0.05
